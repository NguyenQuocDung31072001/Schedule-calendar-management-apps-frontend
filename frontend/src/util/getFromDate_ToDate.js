...
export const getFromDate_ToDate = (_fromDate) => {
  ...
  switch (detailFormDate.month) {
    ...
    default:
      if (
        (detailFormDate.year % 4 === 0 && detailFormDate.year % 100 !== 0) ||
        detailFormDate.year % 400 === 0
      ) {
  ...
  return {
    fromDate: `${detailFormDate.year}-${
      detailFormDate.month < 10
        ? `0${detailFormDate.month}`
        : detailFormDate.month
    }-${detailFormDate.day}`,
